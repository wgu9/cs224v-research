...
**Problem**: Agents drift from original objectives—research on SWE-agent shows 75% of failures relate to scope control breakdown (52% incorrect/overly specific implementations, 23.4% cascading errors), from SWE-agent: Agent-Computer Interfaces Enable Automated Software Engineering NIPS 2024.
...
- **SWE-Bench-CL variant**: Chronological task sequences per repository for evaluating cross-session learning (Q2)

**What SWE-bench Cannot Provide**:
- Interactive user feedback (no real users in loop)
- Explicit task complexity labels in base version (Verified subset has difficulty annotations)
- Time measurements (automated execution)

**Our Split**:
- Training (50): Extract initial patterns, establish baselines
- Validation (50): Week 3 checkpoint—decide if proceed with Q3
- Test (200): Final evaluation (may use SWE-Bench-CL sequences for Q2 testing)
...
- Current SOTA: AutoCodeRover 19.6%, SWE-agent 12.8%
- Our Target: ≥20% (match SOTA), Stretch: ≥25%

**Q1 Metrics**:
- Drift Rate (primary): % actions violating alignment guards (weighted combination of Scope/Plan/Test/Evidence)
- Scope alignment (secondary): % edit actions that modify files in gold patch
...
- Target: Demonstrate statistical significance (t-test, p<0.05)
...
- Significance tests (t-test, p<0.05)
...
- Budget: $15-30 (hybrid strategy to control costs)
...
- **Week 5**: Full evaluation on 200 test tasks
- **Week 6**: Paper + Demo
...
| SWE-agent | ❌ | ❌ | ❌ |
| Devin (proprietary) | Unknown | Unknown | ❌ |
...
- **What it lacks**: No interactive users, no explicit difficulty labels, no time measurements
- **Why SWE-bench for this research**:
  ...
  5. Scale: 300 tasks (SWE-bench Lite) provide statistical power

**Our Data Split**
- **Training set** (50 tasks):
  - Purpose: Extract initial patterns, tune guard weights
  - Selection: Random sample from SWE-bench Lite
  - Usage: Can inspect ground truth for analysis
- **Validation set** (50 tasks):
  - Purpose: Week 3 checkpoint evaluation
  - Selection: Random sample, stratified by difficulty
  - Usage: Decide whether to proceed with Q3
- **Test set** (200 tasks):
  - Purpose: Final evaluation and comparison with baselines
  - Selection: Remaining tasks from SWE-bench Lite
  - Usage: Report primary metrics, no peeking during development
- **Why this split**: 50 training tasks sufficient to extract 10-20 diverse patterns; 50 validation enough for statistical power (Week 3 decision); 200 test provides robust final evaluation
...
- **External baselines** (three levels):
  ...
  2. **Medium baseline**: Static RAG
     ...
     - Expected: ~15% resolve rate
  3. **Strong baseline**: AutoCodeRover (SOTA)
     - Published system with 19.6% resolve rate
...
- **Interpretation**: 20% = agent successfully solves 20 out of 100 tasks
- **Current SOTA**: AutoCodeRover 19.6%, SWE-agent 12.8%
- **Our targets**: ≥20% (match SOTA), ≥25% (stretch)
...
- **Statistical test**: Two-sample t-test (p < 0.05) to establish significance
...
- **Target**: Statistically significant reduction (t-test, p<0.05)
